...
class Huron:
    ...
    def __init__(self, api_key, mix_url):
        r = requests.get(mix_url + '.json',
                               headers={'X-Api-Key': api_key, 'X-Api-Version': API_VERSION}).json()
        self.api_key = api_key
        self.mix = Mix(r['mix']['name'], r['mix']['id'], r['mix']['tracks_count'], r['mix']['cover_urls']['sq500'])
        self.directorio = 'Descargas/' + r['mix']['name'] + '/'
        self.token = requests.get('http://8tracks.com/sets/new.json',
                                  headers={'X-Api-Key': api_key, 'X-Api-Version': API_VERSION}).json()['play_token']
    ...
    def agregar_etiquetas(self, cancion):        
        try: 
            ...
            etiquetas["TIT2"] = mutagen.id3.TIT2(encoding=3, text=cancion.nombre)
            etiquetas["TALB"] = mutagen.id3.TALB(encoding=3, text=cancion.album)
            etiquetas["TPE2"] = mutagen.id3.TPE2(encoding=3, text=cancion.artista)
            etiquetas["TDRC"] = mutagen.id3.TDRC(encoding=3, text=str(cancion.fecha))
    ...
    def ejecutar(self):
        ...
        for i in range(self.mix.canciones):
            ...
            cancion = Cancion(r['set']['track']['id'],
                              r['set']['track']['name'],
                              self.mix.nombre,
                              r['set']['track']['year'],
    ...
    def debug(self, cancion):
        print 'Descargando canci\xc3\xb3n: %s' % str(cancion)


class Cancion:

    def __init__(self, cancion_id, nombre, album, fecha, artista, url, archivo):
...
class Mix:
    ...
    def __init__(self, nombre, mix_id, canciones, cover):
